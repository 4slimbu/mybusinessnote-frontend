...
class MobileLevelLinks extends Component {
  render() {
    ...
    const levelsList = map(levels, (level, key) => {
      ...
      const levelImg =
        levelStatus.completed_percent >= 100 ? level.badge_icon : level.icon;
      const levelUrl = generateAppRelativeUrl(level.slug);

      return (
        <li
          key={level.slug}
          className={classnames("", { active: currentLevel.id === level.id })}
        >
          <Link
            data-toggle="tab"
            onClick={(e) => onClickLevelLink(e, level)}
            to={levelUrl}
          >
            {levelStatus.completed_percent >= 100 ? (
              ...
              <div>
                <div
                  ...
                  data-percent={level.completed_percent}
                  data-duration="1000"
                  data-color="#6f6170,#bf696e"
                >
                  <img src={levelImg} alt="" />
                </div>
              </div>
      ...
      return (
        <li
          ...
          className={classnames("dropdown", {
            "active open": currentLevel.id === level.id,
          })}
        >
          <Link
            onClick={(e) => onClickLevelLink(e, level)}
          ...
          {!currentLevel.is_down && (
            <ul className="dropdown-menu">{sectionsList(level)}</ul>
          )}
        </li>
      );
    });
          ...
          <div className="container">
            ...
            >
              ...
              {isItemLoaded(hamLevelList) && (
                <ul className="nav navbar-nav">{hamLevelList}</ul>
              )}
            </div>
          </div>
        </nav>
        {isItemLoaded(levels) && <ul className="nav nav-tabs">{levelsList}</ul>}
      </div>
    );
  }
}
