...
class SignUpForm extends Component {
  ...
  onSubmit(e) {
    ...
    const rulesForChangedFields = getRulesForChangedFields(
      this.rules,
      changedFields
    );
    ...
    if (
      ...
      this.isFormValid(allFields, rulesForChangedFields)
    ) {
      makeRequest(request.Auth.save, changedFields, {
      ...
      !auth.isAuthenticated &&
      this.isFormValid(allFields, this.rules)
    ) {
      makeRequest(request.Auth.register, changedFields, {
        message: MESSAGES.REGISTERING,
      }).then(
        (responseData) => {
          makeRequest(
            request.Business.getStatus,
            {},
            { message: MESSAGES.REFRESHING }
          );
          history.push(ROUTES.ADD_BUSINESS_DETAILS);
        },
        (errorData) => {
  ...
  checkIfUserExists(e) {
    ...
    if (isValid) {
      this.props
        .makeRequest(request.Auth.checkIfExists, data, {
          message: MESSAGES.LOADING_USER_CHECK,
        })
        .then(
          (responseData) => {
  ...
  render() {
    ...
    return (
      <form className="apps-form" onSubmit={this.onSubmit}>
        <TextFieldGroup
          fieldObject={this.state.first_name}
          onChange={this.onChange}
          error={errors.first_name}
        />
        <TextFieldGroup
          fieldObject={this.state.last_name}
          onChange={this.onChange}
          error={errors.last_name}
        />
        <TextFieldGroup
          fieldObject={this.state.email}
          onChange={this.onChange}
        ...
        />
        <TextFieldGroup
          fieldObject={this.state.phone_number}
          onChange={this.onChange}
        ...
        {!auth.isAuthenticated && (
          <div>
            <TextFieldGroup
              fieldObject={this.state.password}
              onChange={this.onChange}
              error={errors.password}
            />
            <TextFieldGroup
              fieldObject={this.state.confirm_password}
              onChange={this.onChange}
