...
export function toCapitalizedWords(string) {
  const newString = string.replace(/([A-Z])/g, " $1").replace(/([_])/g, " ");
...
export function extractLevelFromLocation(location) {
  ...
  return (
    params &&
    params.indexOf("level") >= 0 &&
    params[params.indexOf("level") + 1]
  );
}
...
export function extractSectionFromLocation(location) {
  ...
  return (
    params &&
    params.indexOf("section") >= 0 &&
    params[params.indexOf("section") + 1]
  );
}
...
export function extractBoIdFromLocation(location) {
  ...
  let filteredIndexes = splittedArray
    .map(function (c, i) {
      if (c === "bo") {
        return i + 1;
      }
...
export function generateLevelCompletedPercent(levels, currentLevel) {
  return levels && currentLevel && levels[currentLevel.id - 1]
    ? levels[currentLevel.id - 1].completed_percent
    : false;
}
...
export function saveBusinessOption(currentObject, data) {
  currentObject.props.saveBusinessOptionFormRequest(data).then(
    (response) => {
      ...
      currentObject.props
        .getBusinessOptionFromUrl(appStatus.currentBusinessOption.links.self)
        .then((res) => {
          if (response.data.completed_status.level) {
            currentObject.props.history.push(
              "/level/" + appStatus.currentBusinessOption.level_slug
            );
            return;
          }
          ...
          setTimeout(function () {
            currentObject.props.getBusinessOptionFromUrl(
              appStatus.currentBusinessOption.links.next
            );
            currentObject.props.history.push(
              getAppUrlFromApiUrl(appStatus.currentBusinessOption.links.next)
            );
          }, 0);
        });
    },
...
export function isLevelLocked(appStatus, level) {
  ...
  let completedPercent = levelStatus.completed_percent
    ? levelStatus.completed_percent
    : 0;
  ...
  let previousLevelStatus = getStatus(
    appStatus.businessStatus.levelStatuses,
    level.id - 1
  );
  let previousLevelCompletedPercent = previousLevelStatus.completed_percent
    ? previousLevelStatus.completed_percent
    : 0;
  if (previousLevelCompletedPercent >= 100) {
    return false;
  }

  let statusObject = find(
    appStatus.businessStatus.businessOptionStatuses,
    (item) => {
...
export function filterBusinessOptionsBySection(appStatus, sectionId) {
  let relevantBusinessOptions = filter(appStatus.businessOptions, function (
    item
  ) {
    let statusObject = getStatus(
      appStatus.businessStatus.businessOptionStatuses,
      item.id
    );
    return (
      item.section_id === sectionId &&
      statusObject.status != "irrelevant" &&
      !item.parent_id
    );
  });
...
export function hasChildBusinessOptions(appStatus, currentBusinessOption) {
  let relevantBusinessOptions = filter(appStatus.businessOptions, function (
    ...
    return (
      item.parent_id === currentBusinessOption.id &&
      statusObject.status != "irrelevant"
    );
  });
...
export function hasInCompleteChildBusinessOptions(
  appStatus,
  ...
  if (isItemLoaded(relevantBusinessOptions)) {
    let completedBusinessOptions = filter(relevantBusinessOptions, function (
      item
    ) {
      let statusObject = getStatus(
        appStatus.businessStatus.businessOptionStatuses,
...
export function getSectionPosition(
  sectionIdentifierObjectCollection,
  sectionId
) {
...
export function dashboardUrl() {
  if (localStorage.getItem("jwtToken")) {
    return (
      getEnv("DASHBOARD_URL") + "/?token=" + localStorage.getItem("jwtToken")
    );
  }
  return "#";
}
...
export function generateAppRelativeUrl(
  level,
  section = null,
  businessOption = null
) {
